    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM arqmad.exe");
#else
                QProcess::execute("pkill arqmad");
#endif
            }
...
#ifdef Q_OS_WIN
    m_monerod = QApplication::applicationDirPath() + "/arqmad.exe";
#elif defined(Q_OS_UNIX)
    m_monerod = QApplication::applicationDirPath() + "/arqmad";
#endif
