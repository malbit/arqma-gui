...
ARQMA_DIR=$ROOT_DIR/arqma
BUILD_LIBWALLET=false

# init and update arqma submodule
if [ ! -d $ARQMA_DIR/src ]; then
    git submodule init arqma
fi
git submodule update --remote
git -C $ARQMA_DIR fetch
git -C $ARQMA_DIR checkout android
...
OLD_GIT_USER=$(git -C $ARQMA_DIR config --local user.name)
OLD_GIT_EMAIL=$(git -C $ARQMA_DIR config --local user.email)
git -C $ARQMA_DIR config user.name "Arqma GUI"
git -C $ARQMA_DIR config user.email "gui@arqma.local"
# check for PR requirements in most recent commit message (i.e requires #xxxx)
for PR in $(git log --format=%B -n 1 | grep -io "requires #[0-9]*" | sed 's/[^0-9]*//g'); do
    echo "Merging arqma push request #$PR"
    # fetch pull request and merge
    git -C $ARQMA_DIR fetch origin pull/$PR/head:PR-$PR
    git -C $ARQMA_DIR merge --quiet PR-$PR  -m "Merge arqma PR #$PR"
...
## Compare previously built version with submodule + merged PR's version.
    ...
    get_tag
    popd
    echo "latest libwallet version: $GUI_ARQMA_VERSION"
    echo "Installed libwallet version: $VERSIONTAG"
    # check if recent
    if [ "$VERSIONTAG" != "$GUI_ARQMA_VERSION" ]; then
        echo "Building new libwallet version $GUI_ARQMA_VERSION"
        BUILD_LIBWALLET=true
    else
        echo "latest libwallet ($GUI_ARQMA_VERSION) is already built. Remove arqma/lib/libwallet_merged.a to force rebuild"
    fi
fi
...
echo "GUI_ARQMA_VERSION=\"$VERSIONTAG\"" > $ARQMA_DIR/version.sh
...
echo "cleaning up existing arqma build dir, libs and includes"
rm -fr $ARQMA_DIR/build
rm -fr $ARQMA_DIR/lib
rm -fr $ARQMA_DIR/include
rm -fr $ARQMA_DIR/bin


mkdir -p $ARQMA_DIR/build/$BUILD_TYPE
pushd $ARQMA_DIR/build/$BUILD_TYPE
...
if [ "$platform" == "darwin" ]; then
    ...
    if [ "$STATIC" == true ]; then
        cmake -D CMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE -D STATIC=ON -D ARCH="x86-64" -D BUILD_64=ON -D BUILD_GUI_DEPS=ON -D INSTALL_VENDORED_LIBUNBOUND=ON -D CMAKE_INSTALL_PREFIX="$ARQMA_DIR"  ../..
    else
        cmake -D CMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE  -D BUILD_GUI_DEPS=ON -D INSTALL_VENDORED_LIBUNBOUND=ON -D CMAKE_INSTALL_PREFIX="$ARQMA_DIR"  ../..
    fi
...
## LINUX 64
    ...
    if [ "$ANDROID" == true ]; then
        ...
        mkdir -p $ARQMA_DIR/build/$BUILD_TYPE/translations
        cd $ARQMA_DIR/build/$BUILD_TYPE/translations && cmake ../../../translations && make && cd ..
        cmake -D CMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE -D STATIC=ON -D ARCH="armv7-a" -D ANDROID=true -D BUILD_GUI_DEPS=ON -D USE_LTO=OFF -D INSTALL_VENDORED_LIBUNBOUND=ON -D CMAKE_INSTALL_PREFIX="$ARQMA_DIR" -D CMAKE_SYSTEM_NAME="Android" -D CMAKE_ANDROID_STANDALONE_TOOLCHAIN="${ANDROID_STANDALONE_TOOLCHAIN_PATH}" -D CMAKE_ANDROID_ARM_MODE=ON -D CMAKE_ANDROID_ARCH_ABI="armeabi-v7a" ../..
    elif [ "$STATIC" == true ]; then
        cmake -D CMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE -D STATIC=ON -D ARCH="x86-64" -D BUILD_64=ON -D BUILD_GUI_DEPS=ON -D INSTALL_VENDORED_LIBUNBOUND=ON -D CMAKE_INSTALL_PREFIX="$ARQMA_DIR"  ../..
    else
        cmake -D CMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE -D BUILD_GUI_DEPS=ON -D CMAKE_INSTALL_PREFIX="$ARQMA_DIR"  ../..
    fi
...
## LINUX 32
    ...
    if [ "$STATIC" == true ]; then
        cmake -D CMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE -D STATIC=ON -D ARCH="i686" -D BUILD_64=OFF -D BUILD_GUI_DEPS=ON -D INSTALL_VENDORED_LIBUNBOUND=ON -D CMAKE_INSTALL_PREFIX="$ARQMA_DIR"  ../..
    else
        cmake -D CMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE -D BUILD_GUI_DEPS=ON -D CMAKE_INSTALL_PREFIX="$ARQMA_DIR"  ../..
    fi
...
## LINUX ARMv7
    ...
    if [ "$STATIC" == true ]; then
        cmake -D BUILD_TESTS=OFF -D ARCH="armv7-a" -D STATIC=ON -D BUILD_64=OFF  -D BUILD_GUI_DEPS=ON -D CMAKE_INSTALL_PREFIX="$ARQMA_DIR"  ../..
    else
...
## LINUX other
    ...
    if [ "$STATIC" == true ]; then
        cmake -D CMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE -D STATIC=ON -D BUILD_GUI_DEPS=ON -D INSTALL_VENDORED_LIBUNBOUND=ON -D CMAKE_INSTALL_PREFIX="$ARQMA_DIR"  ../..
    else
        cmake -D CMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE -D BUILD_GUI_DEPS=ON -D CMAKE_INSTALL_PREFIX="$ARQMA_DIR"  ../..
    fi
...
## Windows 32
    ...
    if [ "$STATIC" == true ]; then
        cmake -D CMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE -D STATIC=ON -D BUILD_GUI_DEPS=ON -D CMAKE_INSTALL_PREFIX="$ARQMA_DIR"  ../..
    else
        cmake -D CMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE -D BUILD_GUI_DEPS=ON -D CMAKE_INSTALL_PREFIX="$ARQMA_DIR"  ../..
    fi
fi
...
pushd $ARQMA_DIR/build/$BUILD_TYPE/src/wallet
...
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
    pushd $ARQMA_DIR/build/$BUILD_TYPE/src/daemon
...
# build install epee
eval make -C $ARQMA_DIR/build/$BUILD_TYPE/contrib/epee all install

# install easylogging
eval make -C $ARQMA_DIR/build/$BUILD_TYPE/external/easylogging++ all install
